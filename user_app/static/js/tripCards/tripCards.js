...
document.addEventListener("DOMContentLoaded", function () {  
    // [Az] Function to convert RGB to Hex
        ...
        function updateTextColorForCards() {
            const tripFolders = document.querySelectorAll('.trip-folder');
            
            tripFolders.forEach(folder => {
                const bgColor = window.getComputedStyle(folder).backgroundColor;
                const hexColor = rgbToHex(bgColor);
                const textColor = getContrastColor(hexColor);
        
                // [Az] Select ALL elements with the class inside this folder
                const textElements = folder.querySelectorAll('.trip-textColor');
